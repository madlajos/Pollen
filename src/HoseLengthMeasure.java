...
public class HoseLengthMeasure extends AssignhoseEndsToPollens {
	...
	public static double findOtherEnd(Point currentPosition, Mat mask, Point pointToFind)	//doubleCount számára megtalálja az útvonalat a két végpont közt, az utat 80-ra színezi
	{
		...
		if (num == 0)  //végponthoz értünk
		{
			if ((currentPosition.x == pointToFind.x) && (currentPosition.y == pointToFind.y))	//ha megtaláltuk a másik pollenhez tartozó végpontot akkor az út be van színezve 80-ra és visszatérhetünk.
				return actualLength;
			...
			for (int i = pointsToRecolor.size()-1; i >= 0; i--)
			{
				...
				if ((pointsToRecolor.get(i-1).x ==  branches.get(branches.size() - 1).x) &&
					(pointsToRecolor.get(i-1).y ==  branches.get(branches.size() - 1).y) && 
					 branches.size() !=0)	//csak az utolsó elágazásig állítjuk vissza a színeket
				{
